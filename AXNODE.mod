...
PROCEDURE evaluate_fct(v(mV)) { LOCAL a,b,v2, vnashift

	vnashift = v - 10

	a = q10_1*vtrap1(vnashift)
	b = q10_1*vtrap2(vnashift)
	tau_mp = 1 / (a + b)
	mp_inf = a / (a + b)

	a = q10_1*vtrap6(vnashift)
	b = q10_1*vtrap7(vnashift)
	tau_m = 1 / (a + b)
	m_inf = a / (a + b)

	a = q10_2*vtrap8(vnashift) 
	b = q10_2*bhA / (1 + Exp(-(vnashift+bhB)/bhC)) 
	tau_h = 1 / (a + b)
	h_inf = a / (a + b)
